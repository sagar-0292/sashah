...
let S={
  ...
  color:1,pImg:null,mImg:null,times:[],bsCtx:null,
  confirmCb:null,
};
let deferredInstall=null;

function load(){
  try{const d=localStorage.getItem('mv4');if(d){const p=JSON.parse(d);S.profiles=p.profiles||[];S.sosMeds=p.sosMeds||[];}}catch(e){}
}
function save(){
  try{localStorage.setItem('mv4',JSON.stringify({profiles:S.profiles,sosMeds:S.sosMeds}));}catch(e){}
}

// ─── TODAY KEY ───
function today(){return new Date().toISOString().slice(0,10);}
function doseKey(mid,time){return `done_${mid}_${today()}_${time}`;}
...
function renderSOS(){
  ...
  let ha='';
  if(!S.sosMeds.length){
    list.innerHTML=`<div class="empty"><div class="empty-ico" style="color:var(--accent)">${I.pill}</div><h3>No SOS Medicines</h3><p>Add critical medicines to monitor here</p></div>`;
...
function mcard(m,type,pid){
  ...
  if(type!=='sos'&&(m.schedule||[]).length){
    const pills=(m.schedule||[]).map(t=>{
      ...
      const ico=done?I.chk:(past?`<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>`:I.clk);
      return `<div class="dpill ${cls}" onclick="toggleDose('${m.id}','${t}','${tid}')" title="${done?'Mark as not taken':'Mark as taken'}">
        <div class="dpill-ico">${ico}</div>
        ${fmt(t)}
      </div>`;
    }).join('');
    doseSec=`<div class="dose-section"><div class="dose-label">Today's Doses — tap to mark taken</div><div class="dose-pills">${pills}</div></div>`;
  }
...
function toggleDose(mid,time,tid){
  ...
  if(isDone(mid,time)){
    // Undo: give tablet back
    m.count+=1;
    markUndone(mid,time);
    toast('Dose unmarked — tablet returned');
  }else{
    // Mark taken: remove tablet
...
function confirmDeleteProfile(id){
  ...
  S.confirmCb=()=>{
    ...
    save();renderAll();showPage('home');toast(`${p.name} removed`);
  };
...
function openAddMed(type,pid){
  S.editMId=null;S.mImg=null;S.times=[];S.target={type,pid:pid||null};
...
function saveMed(){
  ...
  const d={
    ...
    schedule:[...S.times],img:S.mImg||null,
  };
...
function autoDecrement(){
  const now=new Date(),dk=today();
  S.profiles.forEach(p=>(p.medicines||[]).forEach(m=>(m.schedule||[]).forEach(t=>{
    const[h,mi]=t.split(':').map(Number),sd=new Date();sd.setHours(h,mi,0,0);
    // Only auto-decrement if not already manually marked
    ...
    if(!localStorage.getItem(autoKey)&&!localStorage.getItem(doneKey)&&sd<=now&&m.count>0){
      ...
      localStorage.setItem(doneKey,'1'); // also mark as taken
    }
  })));
  save();renderAll();
}
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('./sw.js')
      .then(reg => console.log('MedVault SW registered:', reg.scope))
